    ...
    db.collection('recipes').doc(recipeId).get().then(doc => {
      if (doc.exists) {
        ...
        for (i = 0; i < recipe.ing.length; i++) {
          document.getElementById("ingred").innerHTML += '<li>' + recipe.ing_measure[i].toString() + ' ' + recipe.ing_type[i].toString() + ' ' + recipe.ing[i].toString() + '</li><br>';
        }
        for (i = 0; i < recipe.directions.length; i++) {
          document.getElementById("dir").innerHTML += '<li>' + recipe.directions[i] + '</li><br>';
        }
      }
    });
  }
